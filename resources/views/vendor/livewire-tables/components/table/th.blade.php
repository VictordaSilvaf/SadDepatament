...
@if ($component->isTailwind())
    <th scope="col" {{
        $attributes->merge($customAttributes)
            ->class(['px-6 py-3 text-left text-xs font-medium whitespace-nowrap text-zinc-500 uppercase tracking-wider dark:bg-zinc-700 dark:text-zinc-400' => $customAttributes['default'] ?? true])
                ...
                <button
                    ...
                    {{
                        $attributes->merge($customSortButtonAttributes)
                            ->class(['flex items-center space-x-1 text-left text-xs leading-4 font-medium text-zinc-500 uppercase tracking-wider group focus:outline-none dark:text-zinc-400' => $customSortButtonAttributes['default'] ?? true])
