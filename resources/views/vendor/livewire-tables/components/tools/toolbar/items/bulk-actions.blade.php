    ...
    >
        <button
            @class([
                ...
                'inline-flex justify-center w-full rounded-md border border-zinc-300 shadow-sm px-4 py-2 bg-white text-sm font-medium text-zinc-700 hover:bg-zinc-50 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-zinc-700 dark:text-white dark:border-zinc-600 dark:hover:bg-zinc-600' => $component->isTailwind(),
            ])
            type="button"
        ...
        @if($component->isTailwind())
            <div
                ...
                class="origin-top-right absolute right-0 mt-2 w-full md:w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-zinc-100 focus:outline-none z-50"
            >
                <div class="rounded-md bg-white shadow-xs dark:bg-zinc-700 dark:text-white">
                    <div class="py-1" role="menu" aria-orientation="vertical">
                        @foreach ($component->getBulkActions() as $action => $title)
                            <button
                                ...
                                type="button"
                                class="block w-full px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-none focus:bg-zinc-100 focus:text-zinc-900 flex items-center space-x-2 dark:text-white dark:hover:bg-zinc-600"
                                role="menuitem"
                            >
                                <span>{{ $title }}</span>
                            </button>
