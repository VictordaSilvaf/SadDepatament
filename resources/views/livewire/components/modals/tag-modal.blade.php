<div class="col-span-2" x-data="{showModalAddTags: false}">
    <div class="flex justify-between mb-2">
        <label
            class="block mb-2 text-sm font-medium text-zinc-900 dark:text-white">Tags</label>
        <div class="">
            <x-button type="button" @click="showModalAddTags = true"
            ...
            <div class="absolute top-0 left-0 h-screen w-screen bg-black bg-opacity-30 z-40"
                 x-show="showModalAddTags" x-cloak>
                <div class="w-full h-full flex justify-center items-center">
                    <div class="max-w-4xl w-full bg-white dark:bg-zinc-700 shadow-lg p-6 rounded-xl"
                         @click.outside="showModalAddTags = false">
                        <div class="flex justify-between items-center">
                            <h3 class="text-xl dark:text-white">Adicionar tag</h3>

                            <button type="button" @click="showModalAddTags = false">
                        ...
                        <div class="mt-4" x-data="{ showSearch: $wire.entangle('showDropdown').live }">
                            <label for="default-tags-search"
                                   class="mb-2 text-sm font-medium text-zinc-900 sr-only dark:text-white">Buscar</label>
                            <div class="relative">
                                ...
                                <input type="search" id="default-tags-search" wire:model="search"
                                ...
                                <div
                                    ...
                                    @if($tagsSearch)
                                        @foreach($tagsSearch as $key => $tag)
                                            <button type="button" wire:click="addNewTag({{$model}}, {{$tag}})"
                            ...
                            @isset($model)
                                <div class="flex flex-wrap gap-2">
                                    @foreach($model->tags as $tag)
                                        <x-global.badges :remove="true" wire:click="removeTag({{ $tag->id }})"
                                                         :text="$tag->title" :type="$tag->color"/>
                                    @endforeach
                                </div>
                            @endisset
                        </div>
